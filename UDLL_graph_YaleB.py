...
def test_face(Img, Label, CAE, num_class):       
    ...
    for i in range(0,39-num_class): 
        ...
        max_step =  2000#50 + num_class*25# 100+num_class*20
        display_step = 100
        lr = 1.0e-3
        # fine-tune network
        epoch = 0
        while epoch < max_step:
            ...
            if epoch % display_step == 0:
                ...
                y_x, Cksym = post_proC(Coef, label_10_subjs.max(), 12,4)                  
    ...
    m = np.mean(acc_)
    me = np.median(acc_)
    sio.savemat('yaleem1',{'X':embed,'Y':label_10_subjs})
...
if __name__ == '__main__':
    ...
    data = sio.loadmat('YaleBCrop025.mat')
    img = data['Y']
    ...
    CAE.initlization()        
    CAE.restore()
    face_10_subjs = np.array(Img[0:64*(num_class),:])
    face_10_subjs = face_10_subjs.astype(float)  
    ...
    L = D - W
    sio.savemat('YaleB_graph',{'L':L,'W':W})